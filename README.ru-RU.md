...
| Переменная | Описание |
|------------|----------|
| `WEBHOOK_URL` | URL webhook-эндпоинта (включает режим webhook) |
| `WEBHOOK_AUTH` | Схема авторизации: `Basic`, `Bearer` или `Off` (по умолчанию) |
| `WEBHOOK_USERNAME` | Имя пользователя для Basic auth |
| `WEBHOOK_PASSWORD` | Пароль для Basic auth |
| `WEBHOOK_TOKEN` | Токен для Bearer auth |
...
- `payload` — данные OCPP
- `account` — необязательный идентификатор учётной записи (извлекается из URL подключения: путь имеет вид `/ocpp/{account}/{identity}` при трёх сегментах либо `/ocpp/{identity}` при двух — например `ws://host/ocpp/AC0001/EM-A0000001`)

> **Учётная запись идёт первой, идентификатор последним.** Если поменять их
> местами, ничего не сломается: за идентификатор зарядной станции всегда берётся
> последний сегмент, поэтому станция зарегистрируется под значением учётной
> записи и вернёт пустой `account`. Если станция появилась не там, где
> ожидалось, — сверьте поле `identity` в первом же вызове webhook.
