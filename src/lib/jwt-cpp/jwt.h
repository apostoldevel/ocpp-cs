...
namespace jwt {
    ...
    struct signature_exception : public std::runtime_error {
        signature_exception()
                : std::runtime_error("Signature failed.")
        {}
        explicit signature_exception(const std::string& msg)
                : std::runtime_error(msg)
        {}
        explicit signature_exception(const char* msg)
                : std::runtime_error(msg)
        {}
    };

	struct signature_verification_exception : public signature_exception {
		signature_verification_exception()
			: signature_exception("Signature verification failed.")
		{}
		explicit signature_verification_exception(const std::string& msg)
			: signature_exception(msg)
		{}
	...
	struct signature_generation_exception : public signature_exception {
		signature_generation_exception()
			: signature_exception("Signature generation failed.")
		{}
	...
	struct rsa_exception : public signature_exception {
	...
	struct ecdsa_exception : public signature_exception {
	...
	struct token_verification_exception : public std::runtime_error {
		...
		explicit token_verification_exception(const std::string& msg)
			: std::runtime_error("Verification failed: " + msg)
		{}
	};

    struct token_expired_exception : public token_verification_exception {
        token_expired_exception()
                : token_verification_exception("Token expired.")
        {}
        explicit token_expired_exception(const std::string& msg)
                : token_verification_exception("Token expired (" + msg + ").")
        {}
    };
	...
	namespace algorithm {
		...
		public:
            ...
            algorithm(): md(nullptr), alg_name("none") {

            }

            algorithm(const EVP_MD*(*md)(), std::string name): md(md), alg_name(std::move(name)) {
            ...
            virtual void verify(const std::string& data, const std::string& signature) const {
				if (!signature.empty())
		 ...
		 */
		struct hmacsha: public algorithm {
		public:
			/**
			...
			hmacsha(std::string key, const EVP_MD*(*md)(), const std::string& name)
				: algorithm(md, name), secret(std::move(key))
			{}
			/**
			 * Sign jwt data
			 ...
			 */
			void verify(const std::string& data, const std::string& signature) const override {
				try {
					auto res = sign(data);
					bool matched = true;
		...
		public:
			...
			ecdsa(const std::string& public_key, const std::string& private_key, const std::string& public_key_password, const std::string& private_key_password, const EVP_MD*(*md)(), const std::string& name, size_t siglen)
				: algorithm(md, name), signature_length(siglen)
			{
				if (!public_key.empty()) {
					std::unique_ptr<BIO, decltype(&BIO_free_all)> pubkey_bio(BIO_new(BIO_s_mem()), BIO_free_all);
	...
	public:
		 ...
		 */
		std::set<std::string> as_set() const {
	...
	public:
		...
		/**
		 * Get audience claim
		 * \return audience as strings
		 * \throws std::runtime_error If claim was not present
		 * \throws std::bad_cast Claim was present but not a set (Should not happen in a valid token)
		 */
		std::string get_audience() const { return get_payload_claim("aud").as_string(); }
        /**
         * Get audiences claim
         ...
         * \throws std::bad_cast Claim was present but not a set (Should not happen in a valid token)
         */
        std::set<std::string> get_audiences() const {
	...
	public:
		...
		void verify(const decoded_jwt& jwt) const {
			...
			const auto& sig = jwt.get_signature();
			const auto& algo = jwt.get_algorithm();
			...
			auto assert_claim_eq = [](const decoded_jwt& jwt, const std::string& key, const claim& c) {
				if (!jwt.has_payload_claim(key))
					throw token_verification_exception("Missing " + key + " claim");
				const auto& jc = jwt.get_payload_claim(key);
				...
				if (c.get_type() == claim::type::int64) {
					...
					const auto& s1 = c.as_set();
					const auto& s2 = jc.as_set();
					if (s1.size() != s2.size())
						throw token_verification_exception("Claim " + key + " does not match expected");
					auto it1 = s1.cbegin();
					auto it2 = s2.cbegin();
			...
			if (jwt.has_expires_at()) {
				const auto leeway = claims.count("exp") == 1 ? std::chrono::system_clock::to_time_t(claims.at("exp").as_date()) : default_leeway;
				const auto exp = jwt.get_expires_at();
				if (time > exp + std::chrono::seconds(leeway))
					throw token_expired_exception();
			}
			if (jwt.has_issued_at()) {
				const auto leeway = claims.count("iat") == 1 ? std::chrono::system_clock::to_time_t(claims.at("iat").as_date()) : default_leeway;
				const auto iat = jwt.get_issued_at();
				if (time < iat - std::chrono::seconds(leeway))
					throw token_expired_exception();
			}
			if (jwt.has_not_before()) {
				const auto leeway = claims.count("nbf") == 1 ? std::chrono::system_clock::to_time_t(claims.at("nbf").as_date()) : default_leeway;
				const auto nbf = jwt.get_not_before();
			...
			for (auto& c : claims) {
				if (c.first == "exp" || c.first == "iat" || c.first == "nbf") {
					...
					if (c.second.get_type() == jwt::claim::type::array) {
                        const auto& aud = jwt.get_audiences();
                        const auto& expected = c.second.as_set();
                        for (auto& e : expected)
                            if (aud.count(e) == 0)
                                throw token_verification_exception("Token doesn't contain the required audience");
					} else {
                        const auto& aud = jwt.get_audience();
                        if (aud != c.second.as_string())
                            throw token_verification_exception("Token doesn't contain the required audience");
					}
				} else {
