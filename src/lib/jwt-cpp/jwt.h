...
namespace jwt {
	...
	namespace error {
        ...
        struct token_expired_exception : public std::system_error {
            using system_error::system_error;
        };
		/**
		...
		inline std::error_category& token_verification_error_category() {
			...
			public:
				...
				std::string message(int ev) const override {
					...
					case token_verification_error::issuer_missmatch:
						return "token doesn't contain the required issuer";
        ...
        enum class token_expired_error {
            ok = 0,
            token_expired
        };
        /**
         * \brief Error category for token expired error
         */
        inline std::error_category& token_expired_error_category() {
            class token_expired_error_cat : public std::error_category {
            public:
                const char* name() const noexcept override { return "token_expired_error"; };
                std::string message(int ev) const override {
                    switch (static_cast<token_expired_error>(ev)) {
                        case token_expired_error::ok: return "no error";
                        case token_expired_error::token_expired: return "token expired";
                        default: return "unknown token expired error";
                    }
                }
            };
            static token_expired_error_cat cat = {};
            return cat;
        }

		inline std::error_code make_error_code(token_expired_error e) {
			return {static_cast<int>(e), token_expired_error_category()};
		}
		...
		inline void throw_if_error(std::error_code ec) {
			if (ec) {
				...
				if (ec.category() == token_expired_error_category()) throw token_expired_exception(ec);
			}
		}
	} // namespace error
} // namespace jwt
...
namespace std {
	...
	template<>
	struct is_error_code_enum<jwt::error::token_expired_error> : true_type {};
} // namespace std
...
#undef JWT_CPP_SUPPORTS_AS
	...
	public:
		 ...
		 * \throw std::bad_cast Claim was present but not a string (Should not happen in a valid token)
		 */
        typename json_traits::string_type get_audience() const { return get_payload_claim("aud").as_string(); }
		/**
		 * Get expires claim
		...
		struct date_before_claim {
			...
			void operator()(const verify_context<json_traits>& ctx, std::error_code& ec) const {
				...
				if (ctx.current_time > c + std::chrono::seconds(leeway)) {
					ec = error::token_expired_error::token_expired;
				}
			}
		};
